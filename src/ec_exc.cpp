...
void Ec::trace_sysenter(){
    debug_started_trace(0, "SysEnter ARG_IP/RIP %lx:%lx utcb_rip %lx Rdi %lx:%lx", current->regs.ARG_IP, current->regs.REG(ip),
    current->utcb->get_rip(), current->regs.ARG_1, current->regs.REG(di));
    return;
}